...
| Not covered | Why |
| --- | --- |
| `waived` | **No fixture tree carries a `deviations` block.** It is the one of the five states the catalog never exercises, so a claim covering every row still demonstrates four states out of five. The first `waived` anywhere came from a second repository, not from a fixture — see [second-repository-walk.md](records/second-repository-walk.md). |
